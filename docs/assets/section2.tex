...
\textit{Mexican Licence Plate Manager} (MLP-Manager) constituye un módulo complementario, una capa de administración y verificación visual del sistema LPR-Sentinel, diseñada para proporcionar una interfaz gráfica de usuario que permita la gestión integral de la base de datos vehicular sintética generada por MLP-Generator. Este módulo responde a la necesidad de dotar al sistema de una herramienta accesible para la inspección, modificación y consulta de los registros almacenados, extendiendo las capacidades del pipeline original hacia un entorno de administración visual que no depende de consultas manuales mediante línea de comandos o interacciones directas con el motor SQLite.

La arquitectura de MLP-Manager se ha implementado sobre el framework Windows Forms de .NET, empleando el lenguaje C\# y el proveedor oficial de SQLite como puente de comunicación con la base de datos. El módulo se comunica directamente con la base de datos generada por MLP-Generator, interpretando su esquema relacional y presentando su contenido en una disposición tabular que replica su estructura a 8 columnas: Matricula, Estado, Marca/Modelo, Color del vehículo, Estatus Legal, Propietario Virtual, Fecha de registro y Filename.

La interfaz principal del administrador, ilustrada en la Figura \ref{fig_manager_ui}, se organiza en dos regiones. En el sector izquierdo se concentran los controles de operación, agrupando cinco botones de acción principal correspondientes a las operaciones de la base de datos: \texttt{Cargar BD}, \texttt{Agregar}, \texttt{Actualizar}, \texttt{Eliminar} y \texttt{Limpiar} registros, así como un botón de \texttt{Búsqueda} que activa una funcionalidad de consulta específica. El sector derecho, que ocupa aproximadamente el 80\% del área de la ventana, corresponde a un visor tabular de la tabla activa, ofreciendo una representación inmediata del conjunto completo de registros contenidos en la base de datos.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.5]{figures/manager_ui.png}
\caption[Interfaz principal del módulo MLP-Manager]{Interfaz principal del módulo MLP-Manager. La zona izquierda agrupa las operaciones CRUD y consulta, mientras que la zona derecha se despliega el contenido tabular de la base de datos.}
...
Su implementación sobre Windows Forms y C\#, si bien podría parecer una desviación del stack de Python utilizado en el resto de módulos, responde más a nuestras limitantes de conocimiento referentes a la creación de interfaces visualices, mismas que actualmente son precarias con el primero. Sin embargo, esta variante demuestra que el sistema se puede beneficiar de la madurez de otros ecosistemas como lo es .NET, incluso extenderse a implementaciones web donde el control se garantice entre múltiples plataformas en la nube.