...
Para poder abordar esta variedad de condiciones, el sistema propuesto adopta un enfoque basado en datos sintéticos, en lugar de depender de la recolección y etiquetado manual de imágenes reales. Esta decisión resuelve las complejidades éticas y legales asociadas al manejo de datos reales y garantiza el control absoluto sobre la distribución, variabilidad y anotación de los conjuntos de entrenamiento. La arquitectura, una variante que toma como referencia el pipeline en \cite{islam-afroz2025} y refinado con la propuesta de \cite{fastplateOCR}, se desglosa en cinco componentes funcionales independientes como se ilustra en la Figura \ref{fig_pipeline}. Esta adaptación permite abordar de manera particular cada etapa del proceso de reconocimiento, desde la generación de muestras sintéticas hasta la consulta de información vehicular asociada.% Esta modularidad facilita la evaluación aislada de cada componente y la mejora del sistema conforme sea necesario.
...
El algoritmo opera bajo una arquitectura de generación basada en plantillas (\textit{template-based generation}), tomando como referencia una imagen de $500 \times 246$ píxeles por cada estado. Las plantillas han sido adaptadas de información pública \cite{matriculas-mexico} para asegurar el espacio editable de la serie. El módulo permite parametrizar la cantidad de matrículas a generar por estado, ya sea de manera global o por entidad específica, respetando los espacios destinados a los caracteres.
...
El entrenamiento del modelo se realizó utilizando el framework PyTorch para la gestión del conjunto de datos y la aumentación de las muestras. El script de entrenamiento disponible como anexo al presente documento, indica el procedimiento de entrenamiento completo. El modelo base para la detección seleccionado fue YOLOv8n, correspondiente a la variante \quotes{nano} de YOLOv8, que ofrece un equilibrio adecuado entre número de parámetros (aproximadamente 3 millones) y precisión media promedio (mAP) comparada con versiones previas y posteriores en \cite{jiang-zhong2025, jegham-koh2025}, cuyo resultado promete ser suficiente para el despliegue en dispositivos con recursos limitados como la Raspberry Pi o cámaras IP con capacidades de procesamiento embebido.

Por limitaciones de hardware, el entrenamiento se realizó con 95 épocas y un tamaño de imagen de entrada de $640\times 640$ píxeles como prueba de concepto, dimensión estándar en la familia YOLO que permite capturar suficiente detalle contextual para la detección de patrones sin incurrir en un costo computacional excesivo. El conjunto de datos utilizado contiene imágenes de vehículos capturadas en diversas condiciones ambientales, ángulos de cámara y distancias, con anotaciones manuales que delimitan la posición de las matrículas mediante cajas delimitadoras rectangulares similarmente al trabajo de \cite{tomas-alcantara2023}. Durante el proceso de entrenamiento, el modelo optimiza una función de pérdida compuesta que combina el error de localización de las cajas (utilizando pérdida CIoU, \textit{Complete Intersection over Union}), la pérdida de confianza por objeto y la pérdida de clasificación para distinguir la clase \quotes{placa} de cualquier otro objeto presente en la escena. Al finalizar las 95 épocas, los pesos del mejor modelo generado durante estas épocas, aquellos que maximizan la métrica mAP sobre el conjunto de validación, se almacenaron en un archivo de PyTorch dentro del directorio de entrenamiento.
...
%region MLP-Manager
\subsection*{MLP-Manager}
\addcontentsline{toc}{subsection}{MLP-Manager}
\textit{Mexican Licence Plate Manager} (MLP-Manager) constituye un módulo complementario, una capa de administración y verificación visual del sistema LPR-Sentinel, diseñada para proporcionar una interfaz gráfica de usuario que permita la gestión de la base de datos vehicular generada por MLP-Generator. Este módulo dota al sistema de una herramienta accesible para la inspección, modificación y consulta de los registros almacenados, extendiendo las capacidades del pipeline original hacia un entorno de administración visual que no depende de consultas manuales mediante línea de comandos o interacciones directas con el motor SQLite.

La arquitectura de MLP-Manager se ha implementado sobre el framework Windows Forms de .NET, empleando el lenguaje C\# y el proveedor oficial de SQLite como puente de comunicación con la base de datos. El módulo se comunica directamente con la base de datos interpretando su esquema relacional y presentando su contenido en una disposición tabular que replica su estructura a 8 columnas: Matricula, Estado, Marca/Modelo, Color del vehículo, Estatus Legal, Propietario Virtual, Fecha de registro y Filename.

La interfaz principal del administrador, ilustrada en la Figura \ref{fig_manager_ui}, se organiza en dos regiones. En el sector izquierdo se concentran los controles de operación, agrupando cinco botones de acción principal correspondientes a las operaciones de la base de datos: \texttt{Cargar datos}, \texttt{Agregar registro}, \texttt{Actualizar registro}, \texttt{Eliminar registro} y \texttt{Limpiar datos} registros, así como un botón de \texttt{Búsqueda} que activa una funcionalidad de consulta específica. El sector derecho, que ocupa aproximadamente el 80\% del área de la ventana, corresponde a un visor tabular de la tabla activa, ofreciendo una representación inmediata del conjunto completo de registros contenidos en la base de datos.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.5]{figures/manager_ui.png}
\caption[Interfaz principal del módulo MLP-Manager]{Interfaz principal del módulo MLP-Manager. La zona izquierda agrupa las operaciones CRUD y consulta, mientras que en la zona derecha se despliega el contenido tabular de la base de datos.}
\label{fig_manager_ui}
\end{figure}

\subsubsection*{Conectividad}
\addcontentsline{toc}{subsubsection}{Conectividad}
El módulo implementa un patrón de conexión dinámica que permite al operador seleccionar cualquier archivo con extensión \texttt{.db} compatible mediante un cuadro de diálogo estándar de exploración de archivos. La conexión con el motor SQLite se establece exclusivamente durante las operaciones de consulta o modificación de los registros, garantizando la liberación inmediata de recursos siguiendo las mejores prácticas de gestión de memoria en .NET.

La integración con la base de datos sintética no requiere modificaciones en el esquema generado por MLP-Generator. La aplicación obtiene automaticamente la lista de tablas disponibles en la base de datos cargada, y selecciona aquella que corresponde a los registros vehiculares. Esta flexibilidad permite que distintas versiones del dataset, generadas con diferentes parámetros de volumen o distribución estatal, sean cargadas sin requerir modificaciones por parte del administrador.

\subsubsection*{Operaciones de manipulación de datos}
\addcontentsline{toc}{subsubsection}{Operaciones de manipulación de datos}
Las cuatro operaciones fundamentales del CRUD (\textit{Create, Read, Update, Delete}) han sido implementadas siguiendo un patrón de ventanas modales que aislan la lógica de edición del contexto principal de visualización. Con esta implementación se evita la manipulación directa sobre el visualizador, reduciendo la probabilidad de errores de entrada y simplificando la implementación de validaciones complejas.

La operación de lectura se consigue al presionar el botón \texttt{Cargar datos}, el sistema invoca una ventana de diálogo que filtra las extensiones \texttt{.db}, \texttt{.sqlite} y \texttt{.sqlite3}. Una vez seleccionado el archivo a través de esta ventana de exploración de archivos, MLP-Manager construye una conexión con el archivo cargado, abre un canal de comunicación y ejecuta una consulta SQL que arroja como resultado todos los registros de la base de datos. Los resultados se instancian sobre un objeto del tipo \texttt{DataTable}, una clase nativa de C\#, cuyas propiedades permiten estructurar al objeto como una tabla dentro del visualizador.

La operación de creación despliega una ventana secundaria, ilustrada en la Figura \ref{fig_manager_add}, que presenta campos de entrada para las ocho columnas del registro. Los campos de texto que requieren datos genéricos como el nombre del propietario o el número de serie de la matricula se implementan mediante controles \texttt{TextBox}, mientras que el Estatus Legal, el tipo, color y marca del coche emplea un \texttt{ComboBox} con opciones precargadas. El campo de fecha utiliza un \texttt{DateTimePicker} configurado con el formato personalizado \texttt{yyyy-MM-dd}, garantizando la coherencia con el almacenamiento en SQLite. Al presionar el botón \texttt{Guardar}, el formulario ejecuta una sentencia SQL del tipo \texttt{INSERT}, validando previamente que ningún campo obligatorio permanezca vacío. La operación cierra el formulario y notifica al formulario principal mediante una ventana de diálogo, actualizando automáticamente el visualizador para presenciar el cambio.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.5]{figures/manager_add.png}
\caption[Formulario de adición de registros en MLP-Manager]{Formulario de adición de registros en MLP-Manager. Los campos se organizan en un panel a dos columnas, con etiquetas informativas y elementos de entrada. El botón Guardar ejecuta la inserción SQL mientras que el botón Cancelar descarta la operación sin efectos en la base de datos.}
\label{fig_manager_add}
\end{figure}

La operación de actualización sigue un patrón similar a la operación de nuevo registro, con la diferencia de que en este nuevo formulario de edición se precargan los valores actuales de la fila del registro seleccionado (Ver Figura~\ref{fig_manager_update}). Para ello, el administrador debe seleccionar la fila que se desea actualizar mediante un click sobre el primer elemento de la primera fila por actualizar, esto permitirá que todos los elementos de esa misma fila se seleccionen. El formulario de actualización recibe esta fila como parámetro al presionar el botón de actualización, y extrae los ocho valores correspondientes asignándolos a los controles de entrada antes de hacer visible la nueva ventana. Posteriormente el administrador será capaz de modificar manualmente cada campo y, al confirmar los cambios, el sistema ejecuta una sentencia SQL del tipo \texttt{UPDATE}, permitiendo incluso que el usuario modifique el valor de la propia matrícula sin perder la referencia al registro original.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.5]{figures/manager_update.png}
\caption[Formulario de actualización de registros en MLP-Manager]{Formulario de actualización de registros en MLP-Manager. Los campos se organizan en un panel a tres columnas, con etiquetas informativas y elementos de entrada. El botón Actualizar ejecuta la inserción SQL mientras que el botón Cancelar descarta la operación sin efectos en la base de datos.}
\label{fig_manager_update}
\end{figure}

Finalmente, la operación de eliminación de registro implementa una verificación de seguridad mediante un cuadro de diálogo \texttt{MessageBox} con botones \texttt{Yes/No}. Esta confirmación evita eliminaciones accidentales. Una vez confirmada la acción, el sistema ejecuta una operación SQL del tipo \texttt{DELETE}, reportando el éxito mediante otra ventana de diálogo y actualizando automáticamente la tabla para reflejar el estado actualizado.

\subsubsection*{Sistema de consulta por matrícula}
\addcontentsline{toc}{subsubsection}{Sistema de consulta por matrícula}
Una de las funcionalidades más importantes de MLP-Manager es su módulo de búsqueda para la validación de matrículas. Al presionar el botón \texttt{Buscar matrícula} en la interfaz principal, se despliega una nueva ventana independiente dedicada a extraer un registro a partir de su matrícula y presentar todos sus atributos en campos de solo lectura, mientras se reporta la latencia de la consulta.

Su interfaz se organiza en tres regiones: un panel superior que contiene el campo de entrada de texto para la matrícula, el botón de ejecución de búsqueda y un espacio reservado para la visualización de resultados. Cuando el usuario introduce una matrícula y presiona el botón \texttt{Buscar}, el sistema ejecuta la consulta SQL \texttt{SELECT} mediante las funciones nativas de .NET. Internamente, la operación de búsqueda implementa un algoritmo de búsqueda por árbol B+ cuya complejidad temporal es $O(log~n)$, esto implica que el tiempo de procesamiento crece en función de los datos de entrada aproximadamente a una razón logaritmica de la forma $log~n$, lo que permite un tiempo de respuesta entre 1 y 7 ms considerando 8,000 registros disponibles.

Inmediatamente después de recibir la respuesta desde la base de datos, se decodifica el resultado mostrando los valores en las columnas correspondientes si existe coincidencia, o mostrando un mensaje informativo en caso contrario.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.5]{figures/manager_search.png}
\caption[Formulario de búsqueda]{Formulario de búsqueda. El campo de entrada acepta la matrícula objetivo, el botón Buscar ejecuta la consulta parametrizada, y la etiqueta superior muestra el tiempo de respuesta en milisegundos. Los ocho campos inferiores presentan los atributos completos del registro encontrado en modo solo lectura.}
\label{fig_manager_search}
\end{figure}

\subsubsection*{Integración con el sistema LPR-Sentinel}
\addcontentsline{toc}{subsubsection}{Integración con el sistema LPR-Sentinel}
MLP-Manager se considera como un módulo independiente, una funcionalidad administrativa que completa la cadena de herramientas de LPR-Sentinel. Mientras que MLP-Generator provee los datos sintéticos y MLP-Pipeline ejecuta el reconocimiento automático, MLP-Manager ofrece la capacidad de inspección y corrección manual de esos mismos datos. 

Su implementación sobre Windows Forms y C\#, si bien podría parecer una desviación del stack de Python utilizado en el resto de módulos, responde más a nuestras limitantes de conocimiento referentes a la creación de interfaces visuales, mismas que actualmente son precarias con el primero. Sin embargo, esta variante demuestra que el sistema se puede beneficiar de la madurez de otros ecosistemas como lo es .NET, incluso extenderse a implementaciones web donde el control se garantice entre múltiples plataformas en la nube.